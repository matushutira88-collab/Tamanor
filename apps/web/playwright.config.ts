...
export default defineConfig({
  ...
  projects: [
    ...
    { name: "auth-desktop", testMatch: /(authed|inbox)\.spec\.ts/, use: { ...devices["Desktop Chrome"], viewport: { width: 1440, height: 900 }, storageState: STORAGE } },
    { name: "auth-mobile", testMatch: /(authed|inbox)\.spec\.ts/, use: { ...mobile(390, 844), storageState: STORAGE } },
    { name: "auth-mobile-small", testMatch: /(authed|inbox)\.spec\.ts/, use: { ...mobile(375, 812), storageState: STORAGE } },
    { name: "auth-tablet", testMatch: /(authed|inbox)\.spec\.ts/, use: { browserName: "chromium", viewport: { width: 768, height: 1024 }, hasTouch: true, storageState: STORAGE } },
  ],
