...
const STATS: { value: string; label: string; note: string }[] = [
  { value: "6", label: "Platforms, one inbox", note: "Facebook, Instagram, YouTube, LinkedIn, TikTok and Google." },
  ...
  return (
    <main className="gu-dark min-h-dvh bg-[var(--color-bg)] text-[var(--color-fg)]">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-14 px-6 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-28">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              <Link href="/login" className="rounded-xl bg-[var(--color-brand)] px-6 py-3 text-center text-sm font-semibold text-[var(--color-brand-fg)] shadow-[0_0_30px_rgba(25,195,154,0.4)] transition hover:bg-[var(--color-brand-strong)]">
      ...
      {/* Stats band — brand principles as figures. Honest, verifiable facts
          only (no fabricated KPIs, per Guardora ground rules). */}
      ...
      <section id="platforms" className="border-t border-[var(--color-border)]">
        <div className="mx-auto max-w-7xl px-6 py-20 text-center">
          <h2 className="mx-auto max-w-2xl gu-display text-3xl md:text-4xl">{t.platformsSection.title}</h2>
          <p className="mx-auto mt-4 max-w-xl text-[var(--color-muted)]">{t.platformsSection.subtitle}</p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            {PLATFORMS.map((p) => (
              <div key={p.label} className="flex items-center gap-2.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-2.5">
      ...
      <section id="pricing" className="border-t border-[var(--color-border)]">
        <div className="mx-auto max-w-7xl px-6 py-20">
          <div className="mb-12 text-center">
            <h2 className="gu-display text-3xl md:text-4xl">{t.pricing.title}</h2>
            <p className="mt-3 text-[var(--color-muted)]">{t.pricing.subtitle}</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {t.pricing.plans.map((p, i) => {
              const highlight = i === 1;
              const talk = i === 3;
              const note = talk ? t.pricing.talkToUs : t.pricing.comingSoon;
              return (
                <div key={p.name} className={`rounded-3xl border bg-[var(--color-surface)] p-6 ${highlight ? "border-[var(--color-brand)] shadow-[0_0_40px_rgba(25,195,154,0.18)]" : "border-[var(--color-border)]"}`}>
                  {highlight ? (
                    <span className="rounded-full bg-[var(--color-brand-soft)] px-2.5 py-0.5 text-[11px] font-semibold text-[var(--color-brand)]">{t.pricing.popular}</span>
                  ) : (
                  ...
                  <p className="mt-6 text-2xl font-semibold text-[var(--color-brand)]">{note}</p>
                  <Link href={talk ? "/contact" : "/book-demo"} className={`mt-5 block rounded-xl px-4 py-2.5 text-center text-sm font-semibold transition ${highlight ? "bg-[var(--color-brand)] text-[var(--color-brand-fg)] hover:bg-[var(--color-brand-strong)]" : "border border-[var(--color-border-strong)] hover:bg-[var(--color-surface-2)]"}`}>
                    {talk ? t.common.contactSales : t.common.getNotified}
                  </Link>
                </div>
              );
            })}
          ...
          <div className="gu-hero relative overflow-hidden rounded-[32px] border border-[var(--color-border-strong)] bg-[var(--color-surface)] px-8 py-14 text-center">
            ...
            <div className="relative">
              ...
              <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Link href="/login" className="w-full rounded-xl bg-[var(--color-brand)] px-6 py-3 text-center text-sm font-semibold text-[var(--color-brand-fg)] shadow-[0_0_30px_rgba(25,195,154,0.4)] transition hover:bg-[var(--color-brand-strong)] sm:w-auto">{t.common.startFreeTrial}</Link>
