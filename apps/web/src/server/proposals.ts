...
export async function rejectProposal(
  ...
  await maybeReturnItemToTriage(session.tenantId, decision.reputationItemId);
...
export async function executeProposal(
  ...
  // Capability + runtime check — mandatory before any platform execution. The
  // runtime hard-disables actions in every V1.2 mode, so this ALWAYS fails for
  // platform actions (never a real hide/reply/delete, never a fake success).
  ...
  // Success (mock for platform actions; state-only otherwise).
  await prisma.moderationDecision.update({
    where: { id: decision.id },
...
/** If an item has no remaining open proposals, return it to triage. */
async function maybeReturnItemToTriage(
  tenantId: string,
  reputationItemId: string,
): Promise<void> {
  const open = await prisma.moderationDecision.count({
    where: {
      tenantId,
      reputationItemId,
