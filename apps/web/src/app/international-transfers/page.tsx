...
import { internationalTransfers } from "@/content/legal-compliance";

export async function generateMetadata(): Promise<Metadata> {
  const doc = internationalTransfers[await getLocale()];
  return { title: doc.metaTitle, description: doc.metaDescription };
}

export default async function Page() {
  const _lp = await getTL();
  const doc = internationalTransfers[_lp.locale];
  return (
    <MarketingPage
      dict={_lp.t}
      locale={_lp.locale}
