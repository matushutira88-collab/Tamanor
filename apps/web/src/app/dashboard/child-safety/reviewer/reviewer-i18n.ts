...
/**
 * RSC BOUNDARY: the whole `ReviewerCopy` object is passed as a prop into "use client" components
 * (FilterBar, EvidencePanel, ProtectionPlanPanel). React cannot serialize an ordinary function into
 * the RSC payload, so this dictionary must contain STRINGS ONLY — counts and interpolated values are
 * expressed as templates with `{placeholder}` tokens and filled in by `fillCopy` at the call site.
 */
...
export interface ReviewerCopy {
  ...
  /** `resultsOne`/`resultsMany` take `{count}`; `pageTemplate` takes `{page}` and `{total}`. */
  list: { empty: string; emptyHint: string; resultsOne: string; resultsMany: string; pageTemplate: string; prev: string; next: string; search: string; searchPlaceholder: string; clear: string; filters: string };
  ...
  /** `statusTitleTemplate`/`statusBodyTemplate` take `{status}`. */
  actions: { assign: string; assignToMe: string; reassign: string; unassign: string; addNote: string; changeStatus: string; confirm: string; cancel: string; working: string; assigneePlaceholder: string; assignTitle: string; assignBody: string; statusTitleTemplate: string; statusBodyTemplate: string; noteTitle: string; statusConfirmBody: string; markdownHint: string; preview: string; write: string; notePlaceholder: string; save: string };
  ...
  pp: {
    tab: string; noPlan: string; recommendation: string; explanation: string; createBlank: string; createFromRec: string;
    planStatus: Record<string, string>; actionStatus: Record<string, string>; priority: Record<string, string>;
    actionType: Record<string, { title: string; desc: string }>; explain: Record<string, string>; event: Record<string, string>;
    activate: string; complete: string; cancel: string; reopen: string; addAction: string; customTitle: string; customDesc: string;
    assign: string; unassign: string; start: string; block: string; skip: string; reopenAction: string; due: string;
    blockReason: string; completionNote: string; progress: string; timeline: string; confirm: string; cancelBtn: string; working: string;
    readOnly: string; empty: string; overdue: string; blockedLabel: string; completedPct: string; assigneePlaceholder: string; notePlaceholder: string; reasonPlaceholder: string; save: string;
    dashNoPlan: string; dashActive: string; dashOverdue: string; dashBlocked: string; dashCompletedToday: string;
  };
}
...
export const REVIEWER_COPY: Record<Locale, ReviewerCopy> = {
  en: {
    ...
    list: { empty: "No incidents", emptyHint: "No child-safety incidents match your filters.", resultsOne: "{count} incident", resultsMany: "{count} incidents", pageTemplate: "Page {page} of {total}", prev: "Previous", next: "Next", search: "Search", searchPlaceholder: "Incident id or profile id…", clear: "Clear", filters: "Filters" },
    ...
    actions: { assign: "Assign", assignToMe: "Assign to me", reassign: "Reassign", unassign: "Unassign", addNote: "Add note", changeStatus: "Change status", confirm: "Confirm", cancel: "Cancel", working: "Working…", assigneePlaceholder: "Reviewer user id", assignTitle: "Assign reviewer", assignBody: "Assign this incident to a reviewer. This is recorded in the incident's history.", statusTitleTemplate: "{status}?", statusBodyTemplate: "Change this incident's status to “{status}”. This is recorded in the incident's history.", noteTitle: "Add reviewer note", statusConfirmBody: "This is recorded in the incident's history and cannot be undone except by reopening.", markdownHint: "Markdown supported · internal only · cannot be edited or deleted", preview: "Preview", write: "Write", notePlaceholder: "Add an internal note…", save: "Save note" },
    statusTarget: targetEn, tl: tlEn, errors: errEn,
  pp: {
    tab: "Protection plan", noPlan: "No protection plan yet.", recommendation: "Recommended plan", explanation: "Why", createBlank: "Create blank draft", createFromRec: "Create from recommendation",
    planStatus: { draft: "Draft", active: "Active", completed: "Completed", cancelled: "Cancelled", reopened: "Reopened" },
    actionStatus: { pending: "Pending", in_progress: "In progress", blocked: "Blocked", completed: "Completed", skipped: "Skipped", reopened: "Reopened" },
    priority: { low: "Low", normal: "Normal", high: "High", urgent: "Urgent" },
    actionType: { review_account_safety: { title: "Review account safety", desc: "Review the protected profile's account safety settings." }, preserve_evidence: { title: "Preserve evidence", desc: "Capture and preserve relevant evidence for this incident." }, verify_guardian_contact: { title: "Verify guardian contact", desc: "Confirm an authorized guardian's contact details." }, notify_authorized_guardian: { title: "Notify authorized guardian", desc: "Notify the authorized guardian through the sanctioned path." }, restrict_interaction: { title: "Restrict interaction", desc: "Recommend restricting the risky interaction." }, recommend_blocking: { title: "Recommend blocking", desc: "Recommend blocking the involved account." }, recommend_reporting: { title: "Recommend reporting", desc: "Recommend reporting to the appropriate authority (advisory only)." }, escalate_internal_safety: { title: "Escalate internal safety", desc: "Escalate internally to the safety team." }, legal_review: { title: "Legal review", desc: "Request an internal legal review (not legal advice)." }, welfare_check: { title: "Welfare check", desc: "Coordinate an internal welfare check." }, follow_up_review: { title: "Follow-up review", desc: "Schedule a follow-up review of the incident." }, custom_internal_action: { title: "Custom internal action", desc: "A reviewer-defined internal action." } },
    explain: { no_evidence_captured: "No evidence captured yet", guardian_not_notified: "Guardian not yet notified", guardian_notified: "Guardian already notified", urgent_risk: "Urgent risk", elevated_risk: "Elevated risk", already_escalated: "Already escalated" },
    event: { plan_created: "Plan created", plan_activated: "Plan activated", plan_completed: "Plan completed", plan_cancelled: "Plan cancelled", plan_reopened: "Plan reopened", action_added: "Action added", action_assigned: "Action assigned", action_unassigned: "Action unassigned", action_started: "Action started", action_blocked: "Action blocked", action_completed: "Action completed", action_skipped: "Action skipped", action_reopened: "Action reopened", due_date_changed: "Due date changed", priority_changed: "Priority changed" },
    activate: "Activate", complete: "Complete plan", cancel: "Cancel plan", reopen: "Reopen plan", addAction: "Add action", customTitle: "Title", customDesc: "Instruction",
    assign: "Assign", unassign: "Unassign", start: "Start", block: "Block", skip: "Skip", reopenAction: "Reopen", due: "Due",
    blockReason: "Block reason", completionNote: "Completion note", progress: "Progress", timeline: "Plan timeline", confirm: "Confirm", cancelBtn: "Cancel", working: "Working…",
    readOnly: "Read-only", empty: "No actions.", overdue: "overdue", blockedLabel: "blocked", completedPct: "complete", assigneePlaceholder: "Reviewer user id", notePlaceholder: "Internal completion note (optional)", reasonPlaceholder: "Internal block reason (optional)", save: "Save",
    dashNoPlan: "Incidents without plan", dashActive: "Active plans", dashOverdue: "Overdue actions", dashBlocked: "Blocked actions", dashCompletedToday: "Plans completed today",
  },
  evidence: {
  ...
  sk: {
    ...
    list: { empty: "Žiadne incidenty", emptyHint: "Vašim filtrom nezodpovedajú žiadne incidenty ochrany detí.", resultsOne: "{count} incidentov", resultsMany: "{count} incidentov", pageTemplate: "Strana {page} z {total}", prev: "Predchádzajúca", next: "Ďalšia", search: "Hľadať", searchPlaceholder: "ID incidentu alebo profilu…", clear: "Vymazať", filters: "Filtre" },
    ...
    actions: { assign: "Priradiť", assignToMe: "Priradiť mne", reassign: "Priradiť inému", unassign: "Odobrať", addNote: "Pridať poznámku", changeStatus: "Zmeniť stav", confirm: "Potvrdiť", cancel: "Zrušiť", working: "Pracuje sa…", assigneePlaceholder: "ID používateľa recenzenta", assignTitle: "Priradiť recenzenta", assignBody: "Priraďte tento incident recenzentovi. Zaznamená sa to do histórie incidentu.", statusTitleTemplate: "{status}?", statusBodyTemplate: "Zmeniť stav tohto incidentu na „{status}“. Zaznamená sa to do histórie incidentu.", noteTitle: "Pridať poznámku recenzenta", statusConfirmBody: "Zaznamená sa to do histórie incidentu a nedá sa vrátiť späť okrem znovuotvorenia.", markdownHint: "Podporuje Markdown · iba interné · nedá sa upraviť ani zmazať", preview: "Náhľad", write: "Písať", notePlaceholder: "Pridať internú poznámku…", save: "Uložiť poznámku" },
    statusTarget: targetSk, tl: tlSk, errors: errSk,
  pp: {
    tab: "Ochranný plán", noPlan: "Zatiaľ žiadny ochranný plán.", recommendation: "Odporúčaný plán", explanation: "Prečo", createBlank: "Vytvoriť prázdny koncept", createFromRec: "Vytvoriť z odporúčania",
    planStatus: { draft: "Koncept", active: "Aktívny", completed: "Dokončený", cancelled: "Zrušený", reopened: "Znovu otvorený" },
    actionStatus: { pending: "Čaká", in_progress: "Prebieha", blocked: "Blokované", completed: "Dokončené", skipped: "Preskočené", reopened: "Znovu otvorené" },
    priority: { low: "Nízka", normal: "Normálna", high: "Vysoká", urgent: "Naliehavá" },
    actionType: { review_account_safety: { title: "Skontrolovať bezpečnosť účtu", desc: "Skontrolujte nastavenia bezpečnosti účtu chráneného profilu." }, preserve_evidence: { title: "Zabezpečiť dôkazy", desc: "Zachyťte a uchovajte relevantné dôkazy k incidentu." }, verify_guardian_contact: { title: "Overiť kontakt opatrovníka", desc: "Potvrďte kontaktné údaje autorizovaného opatrovníka." }, notify_authorized_guardian: { title: "Informovať opatrovníka", desc: "Informujte autorizovaného opatrovníka schválenou cestou." }, restrict_interaction: { title: "Obmedziť interakciu", desc: "Odporučte obmedzenie rizikovej interakcie." }, recommend_blocking: { title: "Odporučiť blokovanie", desc: "Odporučte zablokovanie zapojeného účtu." }, recommend_reporting: { title: "Odporučiť nahlásenie", desc: "Odporučte nahlásenie príslušnému orgánu (len odporúčanie)." }, escalate_internal_safety: { title: "Interná eskalácia", desc: "Eskalujte interne bezpečnostnému tímu." }, legal_review: { title: "Právne posúdenie", desc: "Požiadajte o interné právne posúdenie (nie je to právna rada)." }, welfare_check: { title: "Kontrola blaha", desc: "Koordinujte internú kontrolu blaha." }, follow_up_review: { title: "Následné posúdenie", desc: "Naplánujte následné posúdenie incidentu." }, custom_internal_action: { title: "Vlastná interná akcia", desc: "Interná akcia definovaná recenzentom." } },
    explain: { no_evidence_captured: "Zatiaľ žiadne dôkazy", guardian_not_notified: "Opatrovník ešte neinformovaný", guardian_notified: "Opatrovník už informovaný", urgent_risk: "Naliehavé riziko", elevated_risk: "Zvýšené riziko", already_escalated: "Už eskalované" },
    event: { plan_created: "Plán vytvorený", plan_activated: "Plán aktivovaný", plan_completed: "Plán dokončený", plan_cancelled: "Plán zrušený", plan_reopened: "Plán znovu otvorený", action_added: "Akcia pridaná", action_assigned: "Akcia priradená", action_unassigned: "Akcia odobraná", action_started: "Akcia začatá", action_blocked: "Akcia blokovaná", action_completed: "Akcia dokončená", action_skipped: "Akcia preskočená", action_reopened: "Akcia znovu otvorená", due_date_changed: "Termín zmenený", priority_changed: "Priorita zmenená" },
    activate: "Aktivovať", complete: "Dokončiť plán", cancel: "Zrušiť plán", reopen: "Znovu otvoriť plán", addAction: "Pridať akciu", customTitle: "Názov", customDesc: "Pokyn",
    assign: "Priradiť", unassign: "Odobrať", start: "Začať", block: "Blokovať", skip: "Preskočiť", reopenAction: "Znovu otvoriť", due: "Termín",
    blockReason: "Dôvod blokovania", completionNote: "Poznámka o dokončení", progress: "Priebeh", timeline: "Časová os plánu", confirm: "Potvrdiť", cancelBtn: "Zrušiť", working: "Pracuje sa…",
    readOnly: "Iba na čítanie", empty: "Žiadne akcie.", overdue: "po termíne", blockedLabel: "blokované", completedPct: "dokončené", assigneePlaceholder: "ID používateľa recenzenta", notePlaceholder: "Interná poznámka (voliteľné)", reasonPlaceholder: "Interný dôvod blokovania (voliteľné)", save: "Uložiť",
    dashNoPlan: "Incidenty bez plánu", dashActive: "Aktívne plány", dashOverdue: "Akcie po termíne", dashBlocked: "Blokované akcie", dashCompletedToday: "Plány dokončené dnes",
  },
  evidence: {
  ...
  de: {
    ...
    list: { empty: "Keine Vorfälle", emptyHint: "Keine Kinderschutz-Vorfälle entsprechen Ihren Filtern.", resultsOne: "{count} Vorfälle", resultsMany: "{count} Vorfälle", pageTemplate: "Seite {page} von {total}", prev: "Zurück", next: "Weiter", search: "Suchen", searchPlaceholder: "Vorfall- oder Profil-ID…", clear: "Löschen", filters: "Filter" },
    ...
    actions: { assign: "Zuweisen", assignToMe: "Mir zuweisen", reassign: "Neu zuweisen", unassign: "Zuweisung aufheben", addNote: "Notiz hinzufügen", changeStatus: "Status ändern", confirm: "Bestätigen", cancel: "Abbrechen", working: "Wird ausgeführt…", assigneePlaceholder: "Prüfer-Benutzer-ID", assignTitle: "Prüfer zuweisen", assignBody: "Diesen Vorfall einem Prüfer zuweisen. Dies wird im Verlauf des Vorfalls erfasst.", statusTitleTemplate: "{status}?", statusBodyTemplate: "Status dieses Vorfalls auf „{status}“ ändern. Dies wird im Verlauf erfasst.", noteTitle: "Prüfernotiz hinzufügen", statusConfirmBody: "Dies wird im Verlauf erfasst und kann nur durch Wiedereröffnen rückgängig gemacht werden.", markdownHint: "Markdown unterstützt · nur intern · kann nicht bearbeitet oder gelöscht werden", preview: "Vorschau", write: "Schreiben", notePlaceholder: "Interne Notiz hinzufügen…", save: "Notiz speichern" },
    statusTarget: targetDe, tl: tlDe, errors: errDe,
  pp: {
    tab: "Schutzplan", noPlan: "Noch kein Schutzplan.", recommendation: "Empfohlener Plan", explanation: "Warum", createBlank: "Leeren Entwurf erstellen", createFromRec: "Aus Empfehlung erstellen",
    planStatus: { draft: "Entwurf", active: "Aktiv", completed: "Abgeschlossen", cancelled: "Abgebrochen", reopened: "Wieder geöffnet" },
    actionStatus: { pending: "Ausstehend", in_progress: "In Bearbeitung", blocked: "Blockiert", completed: "Abgeschlossen", skipped: "Übersprungen", reopened: "Wieder geöffnet" },
    priority: { low: "Niedrig", normal: "Normal", high: "Hoch", urgent: "Dringend" },
    actionType: { review_account_safety: { title: "Kontosicherheit prüfen", desc: "Prüfen Sie die Kontosicherheitseinstellungen des geschützten Profils." }, preserve_evidence: { title: "Beweise sichern", desc: "Erfassen und sichern Sie relevante Beweise für diesen Vorfall." }, verify_guardian_contact: { title: "Kontakt bestätigen", desc: "Bestätigen Sie die Kontaktdaten eines autorisierten Erziehungsberechtigten." }, notify_authorized_guardian: { title: "Erziehungsberechtigten benachrichtigen", desc: "Benachrichtigen Sie den autorisierten Erziehungsberechtigten über den zulässigen Weg." }, restrict_interaction: { title: "Interaktion einschränken", desc: "Empfehlen Sie, die riskante Interaktion einzuschränken." }, recommend_blocking: { title: "Blockieren empfehlen", desc: "Empfehlen Sie, das beteiligte Konto zu blockieren." }, recommend_reporting: { title: "Meldung empfehlen", desc: "Empfehlen Sie eine Meldung an die zuständige Stelle (nur Empfehlung)." }, escalate_internal_safety: { title: "Intern eskalieren", desc: "Eskalieren Sie intern an das Sicherheitsteam." }, legal_review: { title: "Rechtliche Prüfung", desc: "Fordern Sie eine interne rechtliche Prüfung an (keine Rechtsberatung)." }, welfare_check: { title: "Wohlergehensprüfung", desc: "Koordinieren Sie eine interne Wohlergehensprüfung." }, follow_up_review: { title: "Folgeprüfung", desc: "Planen Sie eine Folgeprüfung des Vorfalls." }, custom_internal_action: { title: "Eigene interne Aktion", desc: "Eine vom Prüfer definierte interne Aktion." } },
    explain: { no_evidence_captured: "Noch keine Beweise erfasst", guardian_not_notified: "Erziehungsberechtigter noch nicht benachrichtigt", guardian_notified: "Erziehungsberechtigter bereits benachrichtigt", urgent_risk: "Dringendes Risiko", elevated_risk: "Erhöhtes Risiko", already_escalated: "Bereits eskaliert" },
    event: { plan_created: "Plan erstellt", plan_activated: "Plan aktiviert", plan_completed: "Plan abgeschlossen", plan_cancelled: "Plan abgebrochen", plan_reopened: "Plan wieder geöffnet", action_added: "Aktion hinzugefügt", action_assigned: "Aktion zugewiesen", action_unassigned: "Zuweisung aufgehoben", action_started: "Aktion gestartet", action_blocked: "Aktion blockiert", action_completed: "Aktion abgeschlossen", action_skipped: "Aktion übersprungen", action_reopened: "Aktion wieder geöffnet", due_date_changed: "Fälligkeit geändert", priority_changed: "Priorität geändert" },
    activate: "Aktivieren", complete: "Plan abschließen", cancel: "Plan abbrechen", reopen: "Plan wieder öffnen", addAction: "Aktion hinzufügen", customTitle: "Titel", customDesc: "Anweisung",
    assign: "Zuweisen", unassign: "Zuweisung aufheben", start: "Starten", block: "Blockieren", skip: "Überspringen", reopenAction: "Wieder öffnen", due: "Fällig",
    blockReason: "Blockierungsgrund", completionNote: "Abschlussnotiz", progress: "Fortschritt", timeline: "Plan-Zeitachse", confirm: "Bestätigen", cancelBtn: "Abbrechen", working: "Wird ausgeführt…",
    readOnly: "Schreibgeschützt", empty: "Keine Aktionen.", overdue: "überfällig", blockedLabel: "blockiert", completedPct: "abgeschlossen", assigneePlaceholder: "Prüfer-Benutzer-ID", notePlaceholder: "Interne Abschlussnotiz (optional)", reasonPlaceholder: "Interner Blockierungsgrund (optional)", save: "Speichern",
    dashNoPlan: "Vorfälle ohne Plan", dashActive: "Aktive Pläne", dashOverdue: "Überfällige Aktionen", dashBlocked: "Blockierte Aktionen", dashCompletedToday: "Heute abgeschlossene Pläne",
  },
  evidence: {
