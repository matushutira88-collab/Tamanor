...
export function LabelSelector({
  itemId, labels, allLabels, canAct,
}: { itemId: string; labels: LabelLite[]; allLabels: LabelLite[]; canAct: boolean }) {
      ...
      <div className="flex flex-wrap items-center gap-1">
        {labels.map((l) => (
          <span key={l.id} className="inline-flex items-center gap-1">
            ...
            <button type="button" disabled={pending} aria-label={`Remove label ${l.name}`} data-testid="label-remove"
              className="text-xs text-[var(--color-muted)] hover:text-[var(--color-danger)] disabled:opacity-50"
              onClick={() => run(() => removeLabelAction(itemId, l.id), "Label removed")}>×</button>
          </span>
        ))}
        {labels.length === 0 ? <span className="text-xs text-[var(--color-muted)]">No labels</span> : null}
      </div>
      <div className="flex flex-wrap items-center gap-1.5">
        {available.length ? (
          <select className={sel} disabled={pending} defaultValue="" data-testid="label-add"
            onChange={(e) => { const id = e.target.value; if (id) { run(() => addLabelAction(itemId, id), "Label added"); e.target.value = ""; } }}>
            <option value="">+ Add label…</option>
            {available.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}
          </select>
        ) : null}
        <input value={creating} onChange={(e) => setCreating(e.target.value)} placeholder="New label"
        ...
        <button type="button" disabled={pending || !creating.trim()} data-testid="label-create"
          ...
          onClick={() => { const name = creating.trim(); if (!name) return;
            run(async () => { const r = await createLabelAction(name); if (r.ok && r.id) { setCreating(""); return addLabelAction(itemId, r.id); } return r; }, "Label created"); }}>
          {pending ? "…" : "Create"}
        </button>
      </div>
...
export function LabelManager({ allLabels, canAct }: { allLabels: LabelLite[]; canAct: boolean }) {
  const { pending, msg, run } = useInboxAction();
  ...
  return (
    <details className="mb-3" data-testid="label-manager">
      <summary className="cursor-pointer text-xs font-medium text-[var(--color-muted)]">Manage labels ({allLabels.length})</summary>
      <div className="mt-2 flex flex-col gap-2 rounded-lg border border-[var(--color-border)] p-3">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button type="button" disabled={pending || !name.trim()} data-testid="label-manager-create"
            className="rounded-md border border-[var(--color-border-strong)] px-2 py-1 text-xs font-medium disabled:opacity-50"
            onClick={() => run(async () => { const r = await createLabelAction(name.trim()); if (r.ok) setName(""); return r; }, "Label created")}>Create</button>
        </div>
              ...
              {editing === l.id ? (
                <>
                  ...
                  <button type="button" disabled={pending} className="rounded-md border px-2 py-0.5"
                    onClick={() => run(async () => { const r = await renameLabelAction(l.id, editName.trim()); if (r.ok) setEditing(null); return r; }, "Renamed")}>Save</button>
                  <button type="button" className="text-[var(--color-muted)]" onClick={() => setEditing(null)}>Cancel</button>
                </>
              ) : (
                <>
                  <Badge tone={l.colorKey}>{l.name}</Badge>
                  <button type="button" className="text-[var(--color-muted)] hover:underline" onClick={() => { setEditing(l.id); setEditName(l.name); }}>Rename</button>
                  <button type="button" disabled={pending} data-testid="label-manager-delete"
                    className="text-[var(--color-muted)] hover:text-[var(--color-danger)]"
                    onClick={() => run(() => deleteLabelAction(l.id), "Label deleted")}>Delete</button>
                </>
              )}
            </li>
          ))}
          {allLabels.length === 0 ? <li className="text-[var(--color-muted)]">No labels yet.</li> : null}
        </ul>
        <ActionNotice msg={msg} />
      </div>
    </details>
