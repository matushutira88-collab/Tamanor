...
import {
  resolveProviderCredential, storeProviderCredential, getProviderCredentialStatus,
  ProviderCredentialPurpose, type CredentialConnection,
...
/** Staged policy: is the legacy plaintext-column fallback still permitted? Default TRUE (pre-backfill). */
...
export async function resolveMetaAccessToken(account: MetaTokenAccount): Promise<ResolvedMetaToken | null> {
  // Vault-first: try the long-lived token, then a short-lived access token.
  for (const purpose of [ProviderCredentialPurpose.long_lived_token, ProviderCredentialPurpose.access_token]) {
    ...
    }); // throws VaultDecryptError on a corrupt existing row — intentionally NOT caught here.
    if (secret) return { token: secret, source: "vault" };
  }
  // No vault row for either purpose → staged legacy fallback (existing encrypted columns).
