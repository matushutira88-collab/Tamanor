...
# account-protection + monitored-account-limit suite with the REAL tamanor_app role (so the cross-tenant
# RLS denial is genuinely exercised). Never touches production.
set -euo pipefail

CONTAINER="tamanor-account-protection-pg"
PORT="55453"
PW="acctprot_test_pw"
...
echo "→ running account-protection suite (real tamanor_app role)"
DATABASE_URL="$OWNER_URL" APP_DATABASE_URL="$APP_URL" pnpm exec tsx scripts/account-protection.test.ts
