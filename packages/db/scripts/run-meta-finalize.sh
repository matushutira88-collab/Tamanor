...
# meta-finalize + monitored-account-limit suite with the REAL tamanor_app role (so the cross-tenant
# RLS denial is genuinely exercised). Never touches production.
set -euo pipefail

CONTAINER="tamanor-meta-finalize-pg"
PORT="55457"
PW="acctprot_test_pw"
...
echo "→ running meta-finalize suite (real tamanor_app role)"
DATABASE_URL="$OWNER_URL" APP_DATABASE_URL="$APP_URL" pnpm exec tsx scripts/meta-finalize.test.ts
