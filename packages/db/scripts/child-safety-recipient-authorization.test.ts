...
async function main() {
  ...
  const relA = await systemDb.guardianRelationship.create({ data: { tenantId: famA.id, guardianMembershipId: mGuardA.id, protectedProfileId: pA.id, relationshipType: GuardianRelationshipType.Parent, authorityLevel: GuardianAuthorityLevel.Full, guardianRole: "secondary", status: "verified" } });
