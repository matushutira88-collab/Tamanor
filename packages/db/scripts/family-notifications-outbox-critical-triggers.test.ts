...
async function main() {
  ...
  check("★ (1) outbox supports exactly ten notification types", types.length === 10 && ["family_signal_available", "family_urgent_signal", "family_incident_created", "family_incident_escalated"].every((t) => types.includes(t)));
  check("★ (2) the three deferred types remain fail-closed", ["family_guardian_invitation_expiring", "family_consent_expiring", "family_protection_plan_updated"].every((t) => !(t in OUTBOX_TYPE_SOURCE)));
