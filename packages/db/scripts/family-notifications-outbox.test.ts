...
async function main() {
  ...
  // (52) only the delivery transition wires the enqueue — no other src module imports it.
  const { readFileSync, readdirSync } = await import("node:fs");
  const srcDir = new URL("../src/", import.meta.url).pathname;
  const importers = readdirSync(srcDir).filter((f) => f.endsWith(".ts")).filter((f) => /internal\/family-notification-outbox["']|from ["'][^"']*family-notification-outbox["']/.test(readFileSync(srcDir + f, "utf8")));
  check("★ (52) only child-safety-delivery wires the outbox enqueue (no other trigger)", importers.length === 1 && importers[0] === "child-safety-delivery.ts");
