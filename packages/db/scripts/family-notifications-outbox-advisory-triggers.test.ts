...
async function main() {
  ...
  check("★ (1) exactly six outbox notification types supported", types.length === 6 && ["family_delivery_available", "family_delivery_acknowledged", "family_delivery_declined", "family_guardian_invitation_accepted", "family_authority_changed", "family_recipient_authorization_changed"].every((t) => types.includes(t)));
  const forbidden = ["family_signal_available", "family_urgent_signal", "family_guardian_invitation_expiring", "family_consent_expiring", "family_incident_created", "family_incident_escalated", "family_protection_plan_updated"];
  ...
  check("★ (70) exactly the authorized canonical domain services wire the enqueue", JSON.stringify(importers) === JSON.stringify(["child-safety-consent.ts", "child-safety-delivery.ts", "child-safety-recipient-authorization.ts", "family-invitation.ts"]), importers.join(","));
