...
async function run() {
  ...
  const mkQueue = async (id: string) => {
    const ci = await prisma.contentItem.create({ data: { tenantId: T, brandId: brand.id, connectedAccountId: acct.id, platform: "facebook_page", kind: "comment", externalId: `${id}_c`, text: "x", publishedAt: new Date() } });
    ...
    return prisma.actionQueueItem.create({ data: { id, tenantId: T, brandId: brand.id, itemId: ri.id, category: "profanity", confidence: 0.92, proposedAction: "hide_comment", queueState: "approval_required" } });
  };
  ...
  try {
    ...
    const r1 = await submit(ctx(q1.id), { config: CFG, transport: t1, safety: mkSafety() });
    ...
    const r2 = await submit(ctx(q2.id), { config: CFG, transport: t2, safety: mkSafety() });
    ...
    const r3 = await submit(ctx(q3.id), { config: CFG, transport: t3, safety: mkSafety() });
    ...
    const r4 = await submit(ctx(q4.id), { config: CFG, transport: t4, safety: mkSafety() });
    ...
    const r5 = await submit(ctx(q5.id), { config: CFG, transport: t5, safety: mkSafety() });
    ...
    const r6 = await submit(ctx(q6.id), { config: CFG, transport: t6, safety: mkSafety() });
    ...
    const r7 = await submit(ctx(q7.id, { account: { status: "active", health: "healthy", grantedPermissions: ["pages_manage_engagement"], accessToken: RAW, pageId: "AH_PAGE", externalId: "AH_PAGE", connectionStatus: "needs_reconnect", tokenHealth: "expired" } }), { config: CFG, transport: t7, safety: mkSafety() });
    ...
    for (const cat of ["normal_criticism", "customer_question"]) {
      ...
      const r = await submit(ctx(`no-${cat}` as never, { queueItemId: null, matchedCategory: cat }), { config: CFG, transport: t, safety: mkSafety() });
    ...
    const r9 = await submit(ctx("cap" as never, { queueItemId: null }), { config: CFG, transport: t9, safety: mkSafety({ counts: { dayCount: 0, hourCount: 3, categoryDayCount: 0, consecutiveWithoutReview: 0 } }) });
    ...
    const r9b = await submit(ctx("kill" as never, { queueItemId: null }), { config: CFG, transport: t9b, safety: mkSafety({ accountKillSwitch: true }) });
