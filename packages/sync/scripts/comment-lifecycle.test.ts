...
async function run() {
  ...
  const mkQueue = async (id: string) => {
    ...
    return prisma.actionQueueItem.create({ data: { id, tenantId: T, brandId: brand.id, itemId: ri.id, category: "scam", confidence: 0.95, proposedAction: "hide_comment", queueState: "approval_required" } });
  };
  const cleanup = async () => {
  ...
  try {
    ...
    const rDel = await submit(ctx(acct.id, qDel.id), { config: CFG, transport: tDel, liveAttempt: true });
    ...
    const rHide = await submit(ctx(acct.id, qHide.id), { config: CFG, transport: tHide, liveAttempt: true });
    ...
    const rAlready = await submit(ctx(acct.id, qAlready.id), { config: CFG, transport: tAlready, liveAttempt: true });
