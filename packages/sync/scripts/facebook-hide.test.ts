...
async function run() {
  ...
  const _rb = await prisma.brand.create({ data: { tenantId: T, name: "FB Hide Test Brand" } });
  const _ra = await prisma.connectedAccount.create({ data: { tenantId: T, brandId: _rb.id, platform: "facebook_page", status: "active", mode: "read_only", externalId: `FBH_${_rb.id}`, pageId: `FBH_${_rb.id}` } });
  const submit = async (c: HideContext, o?: Parameters<typeof attemptFacebookHide>[1]) => { await ensureHideTarget(prisma, c, _rb.id, _ra.id); return attemptFacebookHide(c, o); };

  const g1 = await submit(baseCtx(), { config: CFG.default, transport: new MockFacebookHideTransport() });
  ...
  const g2 = await submit(baseCtx(), { config: CFG.fbOff, transport: new MockFacebookHideTransport() });
  ...
  const g3 = await submit(baseCtx(), { config: CFG.dryRun, transport: t3 });
  check("3) DRY_RUN=true → dry_run, transport NOT called", g3.status === "dry_run" && t3.calls.length === 0, `${g3.status}/calls=${t3.calls.length}`);

  const g4 = await submit(baseCtx({ account: { status: "active", health: "healthy", grantedPermissions: ["pages_read_engagement"], externalId: "P1" } }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  check("4) missing permission → blocked/missing_permission", g4.status === "blocked" && g4.reason === "missing_permission", `${g4.status}/${g4.reason}`);

  const g5 = await submit(baseCtx({ account: { status: "mock_connected", health: "healthy", grantedPermissions: ["pages_manage_engagement"], externalId: "P1" } }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  check("5) mock account → blocked/account_is_demo", g5.status === "blocked" && g5.reason === "account_is_demo", `${g5.status}/${g5.reason}`);

  const g6 = await submit(baseCtx({ platform: "instagram_business" }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  ...
  const g7 = await submit(baseCtx({ matchedCategory: "normal_criticism" }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  ...
  for (const cat of ["legal_complaint", "refund_complaint", "safety_claim"]) {
    const g = await submit(baseCtx({ matchedCategory: cat }), { config: CFG.live, transport: new MockFacebookHideTransport() });
    check(`8) ${cat} → blocked/safety_never_autonomous`, g.status === "blocked" && g.reason === "safety_never_autonomous", `${g.status}/${g.reason}`);
  }

  const g9 = await submit(baseCtx({ confidence: 0.6 }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  ...
  const gna = await submit(baseCtx({ mode: "approval", trigger: "autonomous" }), { config: CFG.live, transport: new MockFacebookHideTransport() });
  ...
  const glock = await submit(baseCtx(), { config: CFG.live, transport: tlock });
  ...
  const g10 = await submit(baseCtx(), { config: CFG.liveConfirmed, transport: t10 });
  ...
  const gapDry = await submit(baseCtx({ mode: "approval", trigger: "approval" }), { config: CFG.liveConfirmed, transport: new MockFacebookHideTransport({ ok: true, responseCode: "200" }) });
  ...
  const gap = await submit(baseCtx({ mode: "approval", trigger: "approval" }), { config: CFG.liveConfirmed, transport: gapT, liveAttempt: true });
  ...
  const gdry = await submit(baseCtx(), { config: CFG.dryRun, transport: tdry });
  ...
  const i1 = await submit(idemCtx, { config: CFG.dryRun, transport: ti1 });
  const i2 = await submit(idemCtx, { config: CFG.dryRun, transport: ti2 });
  ...
  const e1 = await submit(exCtx, { config: CFG.liveConfirmed, transport: new MockFacebookHideTransport({ ok: true, responseCode: "200" }), liveAttempt: true });
  const tex2 = new MockFacebookHideTransport({ ok: true, responseCode: "200" });
  const e2 = await submit(exCtx, { config: CFG.liveConfirmed, transport: tex2, liveAttempt: true });
  ...
  const f1 = await submit(failCtx, { config: CFG.liveConfirmed, transport: new MockFacebookHideTransport({ ok: false, errorCode: "generic", errorMessage: "nope" }), liveAttempt: true });
  const tf2 = new MockFacebookHideTransport({ ok: true, responseCode: "200" });
  const f2 = await submit(failCtx, { config: CFG.liveConfirmed, transport: tf2, liveAttempt: true });
  ...
  const f3 = await submit(failCtx, { config: CFG.liveConfirmed, transport: tf3, retry: true, liveAttempt: true });
  ...
  const b1 = await submit(blkCtx, { config: CFG.live, transport: new MockFacebookHideTransport() });
  const b2 = await submit(blkCtx, { config: CFG.live, transport: new MockFacebookHideTransport() });
  ...
  const b3 = await submit(blkCtxGranted, { config: CFG.dryRun, transport: new MockFacebookHideTransport() });
  ...
  const pre = await submit(liveCtx(), { config: CFG.dryRun, transport: preT });
  ...
  const l2 = await submit(liveCtx(), { config: CFG.live, transport: lt2, liveAttempt: true });
  ...
  const l3 = await submit(liveCtx(), { config: CFG.dryRun, transport: lt3, liveAttempt: true });
  ...
  const l4 = await submit(liveCtx({ account: { status: "active", health: "healthy", grantedPermissions: ["pages_read_engagement"], externalId: "P1", pageId: "P1" } }), { config: CFG.liveConfirmed, transport: lt4, liveAttempt: true });
  ...
  const l5 = await submit(liveCtx({ itemId: "L_NC", queueItemId: "L_NCQ", matchedCategory: "normal_criticism" }), { config: CFG.liveConfirmed, transport: lt5, liveAttempt: true });
  ...
  for (const cat of ["legal_complaint", "refund_complaint", "safety_claim", "customer_question"]) {
    ...
    const l = await submit(liveCtx({ itemId: `L_${cat}`, queueItemId: `L_${cat}Q`, matchedCategory: cat }), { config: CFG.liveConfirmed, transport: t, liveAttempt: true });
  ...
  const l8 = await submit(liveCtx(), { config: CFG.liveConfirmed, transport: lt8, liveAttempt: true });
  ...
  const l7 = await submit(liveCtx(), { config: CFG.liveConfirmed, transport: lt7, liveAttempt: true });
  ...
  await submit(fCtx, { config: CFG.dryRun, transport: new MockFacebookHideTransport() });
  const lf = await submit(fCtx, { config: CFG.liveConfirmed, transport: new MockFacebookHideTransport({ ok: false, errorCode: "generic", errorMessage: "nope" }), liveAttempt: true });
  ...
  const lfNoRetry = await submit(fCtx, { config: CFG.liveConfirmed, transport: tNoRetry, liveAttempt: true });
  ...
  const lfRetry = await submit(fCtx, { config: CFG.liveConfirmed, transport: tRetry, liveAttempt: true, retry: true });
  ...
  const l17 = await submit(liveCtx({ itemId: "L_D", queueItemId: "L_DQ" }), { config: CFG.default, transport: lt17, liveAttempt: true });
  ...
  const l18 = await submit(liveCtx({ itemId: "L_IG", queueItemId: "L_IGQ", platform: "instagram_business" }), { config: CFG.liveConfirmed, transport: lt18, liveAttempt: true });
  ...
  await prisma.actionQueueItem.deleteMany({ where: { brandId: _rb.id } });
  await prisma.connectedAccount.deleteMany({ where: { id: _ra.id } });
  await prisma.brand.deleteMany({ where: { id: _rb.id } });
  await cleanup();
