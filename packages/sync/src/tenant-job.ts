/**
 * V1.58.8 — trusted tenant job contract + per-tenant execution wrapper (shared library).
 *
 * Relocated into @guardora/sync so the Vercel-native cron job routes (apps/web) can run tenant-scoped
 * maintenance the SAME way the retired persistent worker did — WITHOUT a long-running process. The
 * security model is unchanged:
 *   system discovery (owner) → trusted TenantWorkerJob → runTenantJob → withTenant(job.tenantId) [RLS].
 *
 * A job's `tenantId` MUST originate from a system discovery query or an internal server job — NEVER
 * from an unverified provider/webhook payload. Every tenant read/write inside the wrapper runs under
 * the job's tenant context, so RLS isolates it even if a `where:{tenantId}` is forgotten. Provider
 * HTTP must happen OUTSIDE the `withTenant` callbacks (short read tx → HTTP → short write tx).
 */
...
/** Discriminated union of trusted jobs. Extend as new job types appear. */
...
/** Normalized, non-revealing job error reasons (never contains secrets). */
...
/**
 * Run tenant-scoped work for a validated job. Verifies the job, confirms the tenant exists (under RLS —
 * a foreign/absent tenant is rejected), then hands a tenant tx to `fn`. Errors are normalized to a safe
 * reason and never rethrown. `fn` receives a SHORT tenant tx — no provider HTTP inside it.
 */
